...
static int max17055_exit_hibernate(struct max17055_data *priv)
{
	...
	if (max17055_reg_write(priv, SOFT_WAKEUP, 0x90)) {
		return -EIO;
	}
	if (max17055_reg_write(priv, HIB_CFG, 0x0)) {
		return -EIO;
	}
	if (max17055_reg_write(priv, SOFT_WAKEUP, 0x0)) {
		return -EIO;
	}
				 ...
				 const struct max17055_config *const config)
{
	...
	if (max17055_reg_write(priv, MODEL_CFG, 0x8000)) {
		return -EIO;
	}

	uint16_t model_cfg = 0x8000;
